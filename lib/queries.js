...
module.exports = {
  ...
  getSelectivity: function(val1, va2, selectedSportId){
    return knex('colleges').whereBetween(enrollment, [val1, val2]).andWhere('sport_id', selectedSportId).pluck('selectivity')
    // gets schools where sports id and enrollment between selected values and outputs their locations
  },
  getLocation: function(val1, va2, selectedSportId, selectedSelectivity){
    return knex('colleges').whereBetween(enrollment, [val1, val2]).andWhere({
