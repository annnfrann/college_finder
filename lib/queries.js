...
module.exports = {
  ...
  getLocation: function(val1, val2, selectedSportId, selectedSelectivity){
    return knex('college_sports').where('sport_id', selectedSportId).pluck('college_id').then(function(college_ids){
      return knex('colleges').whereBetween('enrollment', [val1, val2]).whereIn('id', college_ids).whereIn(
        'selectivity', selectedSelectivity
      ).pluck('state')
    })
    // gets school where sports id and 'enrollment' between selected values and locations
  },
  getResults: function(val1, val2, selectedSportId, selectedSelectivity, selectedStates){
    return knex('college_sports').where('sport_id', selectedSportId).pluck('college_id').then(function(college_ids){
      return knex('colleges').whereBetween('enrollment', [val1, val2]).whereIn('id', college_ids).whereIn('state', selectedStates).whereIn('selectivity', selectedSelectivity)
    })
  }
